...
Previous to this research, there has been attempt to undestand the human milk fat layer transcriptome with microarray techniques [@Maningat2009]. This paper analyses the transcriptome with RNA-seq which, compared to microarray, has highly sensitive detail, can accurately quantify both very low and very high abundance transcripts, as well as detect novel transcripts.
...
```{r sampleClusteringTechnique, fig.height=5, fig.width=8, dpi=100, echo=FALSE, fig.cap="Hierarchical clustering of the samples by technical factor - protocol used."}
par(mar=c(8, 5, 1, 1))
...
```{r sampleClusteringLacStage, fig.height=5, fig.width=8, dpi=100, echo=FALSE, fig.cap="Hierarchical clustering of the samples by biological factor - lactation stage."}
par(mar=c(8, 5, 1, 1))
logCPM <- cpm(dge.filt, log=TRUE, prior.count=3)
d <- as.dist(1-cor(logCPM, method="spearman"))
sampleClustering <- hclust(d)
...
```{r mdsPlot, fig.height=5, fig.width=8, dpi=100, echo=FALSE, fig.cap="Figure S7: Multidimensional scaling plot of the samples. Labels correspond to lactation stage and colors indicate protocol used."}
...
Given that the sample size is small, the fold-change plots have wider distribution - because of this we also expect to end up with more DE genes in the DE analysis later. 

In Figure \@ref(fig:TraColfig) we analyse the fold-change for the Colostral and Transitional stage. 
...
From the three plots above (Figures \@ref(fig:TraColfig), \@ref(fig:MatColfig), \@ref(fig:TraMatfig)), we can see that when Colostrum is compared with another stage the difference in expression is greater, than when we compare Transitional and Mature (Figure \@ref(fig:TraMatfig)).
...
In the volcano plot below (Figure \@ref(fig:CTvolc)) we analyse the DE genes between Colostrum and Transitional.
...
```
Keeping the intersection of both DE analysis.
...
The number of DE genes between Colostral-Transitional is `r length(DEgenesEGs_no3)`.

## DE Between Transitional and Mature

In Figure \@ref(fig:TMhist) we analyse the differential expression between the Transitional and Mature stage.
...
In the volcano plot below (Figure \@ref(fig:TMvolc)) we analyse the DE genes between Transitional and Mature

```{r TMvolc, fig.cap="P-value diagnostic plot for differental expression between Transitional and Mature"}
sv <- sva(assays(se.filt.all)$logCPM, mod=mod, mod0=mod0)
mod <- cbind(mod, sv$sv)
colnames(mod) <- c(colnames(mod)[1:2], paste0("SV", 1:sv$n))
fit3 <- lmFit(assays(se.filt.all)$logCPM, mod)
...
The number of DE genes between Transitional-Mature is `r length(DEgenesEGs_no1)`.

## DE Between Colostrum and Mature

In Figure \@ref(fig:CMhist) we analyse the differential expression between the Colostrum and Mature stage.
...
### Volcano plot
In the volcano plot below (Figure \@ref(fig:CMvolc)) we analyse the DE genes between Colostrum and Mature

```{r CMvolc, fig.cap="P-value diagnostic plot for differental expression between Colostrum and Mature"}
sv <- sva(assays(se.filt.all)$logCPM, mod=mod, mod0=mod0)
mod <- cbind(mod, sv$sv)
colnames(mod) <- c(colnames(mod)[1:2], paste0("SV", 1:sv$n))
fit3 <- lmFit(assays(se.filt.all)$logCPM, mod)
...
The number of DE genes between Colostral-Mature is `r length(DEgenesEGs_no2)`.
...
It is evident that the number of differentially expressed genes that was obtained in each of the three analysis is quite in line with the number that was reported in the paper. There the observed numbers in the paper are 8015, 1948 and 8527 respectively - correlation value with our findings `r correlation_value_with_paper`. These numbers are quite large and we think it makes sense because of the nature of the tissue, since the milk should adapt to the infant's needs.
...
In this part we will perform the functional analysis (FA). The scheme of the analysis is similar to the DE analysis. First, we will observe stage 1 and 2 that correspond to colostrum and transitional stage, leaving out the mature stage (stage 3). Afterwards, we will exclude colostrum stage (stage 1) and finally we will exclude transitional stage (stage 2).

For each of the compared stages we will perform 2 FAs: one for GO enrichment and another for KEGG enrichment. The reasons for also doing a KEGG analysis are two:
  (1) Following what the authors did in the paper. 
  (2) GO enrichment analyzes gene sets based on functional annotations from the Gene Ontology, covering a wide range of biological processes - this leaves us with a huge list of enriched pathways. Whereas, KEGG enrichment specifically focuses on identifying overrepresented pathways from the Kyoto Encyclopedia of Genes and Genomes, resulting in a narrower set of enriched pathways compared to the broader functional annotations provided by GO.
  
Finally, for each pair of stages, we will also look at the the top 7 fold-change genes previously extracted and their presence in the enriched gene sets. (This is more of an analysis that we simply did becuase of interest in how much the enrichemnt is related to the fold-change of a gene.)
...
With some trial and error, we decided to set the size to be [3, 100] and the count >= 25. The size represents the number of genes in the GO terms, and the count is the number of genes that we calculate as DE. This way we include the important but not too general pathways; we are furthermore including only the pathways that have a high number of DE genes in the dataset, ei. the gene sets that are highly enriched.
...
In the Table \@ref(tab:CTGOtab) below, the top 10 GO results (sorted by Odds Ratio) are displayed:

```{r CTGOtab, echo=FALSE} 
...
So far we have only included the KEGG IDs. The following 2 functions (getKEGGName and getKEGGClass) add the KEGG name and class to each row. (Note: some are unavailable and so are labeled "no info".)
...
With some trial and error, we decided to set the size to be less than 100 as to not include too general terms.
...
In the Table \@ref(tab:CTKEGGtab) below, the top 10 KEGG results (sorted by Odds Ratio) are displayed:

```{r CTKEGGtab, echo=FALSE}
...
### Top 7 fold-change in GO enrichment 

In the Table \@ref(tab:TOP7CTGO) we see the GO terms related to the top 7 fold-change genes between Colostrum and Transitional.
...
### Top 7 fold-change in KEGG enrichment

In the Table \@ref(tab:TOP7CTKEGG) we see the KEGG terms related to the top 7 fold-change genes between Colostrum and Transitional.
...
With some trial and error, we decided to set the size to be [3, 100] and the count >= 15, for the same reasons mentioned before.
...
In the Table \@ref(tab:TMGOtab) below, the top 10 GO results (sorted by Odds Ratio) are displayed:

```{r TMGOtab, echo=FALSE}
...
Adding the KEGG name and class to each row.
...
With some trial and error, we decided to set the size to be less than 130 as to not include too general terms.
...
In the Table \@ref(tab:TMKEGGtab) below, the top 10 KEGG results (sorted by Odds Ratio) are displayed:

```{r TMKEGGtab, echo=FALSE}
...
In the code block below we see the GO terms related to the top 7 fold-change genes between Transitional and Mature. We don't display the table, because it's empty.
...
In the code block below we see the KEGG terms related to the top 7 fold-change genes between Transitional and Mature. We don't display the table, because it's empty.
...
In the Table \@ref(tab:CMGOtab) below, the top 10 GO results (sorted by Odds Ratio) are displayed:

```{r CMGOtab, echo=FALSE}
...
In the Table \@ref(tab:CMGKEGGtab) below, the top 10 KEGG results (sorted by Odds Ratio) are displayed:

```{r CMGKEGGtab, echo=FALSE}
...
In the Table \@ref(tab:TOP7CMGO) we see the GO terms related to the top 7 fold-change genes between Colostrum and Mature.
...
In the Table \@ref(tab:TOP7CMKEGG) we see the KEGG terms related to the top 7 fold-change genes between Colostrum and Mature.
...
## Interpreting findings of DE and FA between Colostrum and Transitional

The first most significant hit in the KEGG Enrichment table made by comparing Colostrum and Transitional stage suggest that Lipid Metabolism is quite important in the first stage of lactation (\@ref(tab:CTKEGGtab)). The corresponding term for this KEGG class name is fatty acid biosynthesis which indicates that the early synthesis of fatty acids is crucial and beneficial for the milk production. Literature suggest that breastfeeding has a fundamental role in nourishing offspring by supplying a lipid-rich product that is used to create heat and metabolic fuel. Lipids are therefore a necessity for the newborn mammals since they provide heat mediated by the uncoupling of mitochondrial respiration in specific fat depots[@Rszer2021]. Additionally, fatty acid elongation has been found as significant KEGG term for the same lactation stage along with glycosphingolipid biosynthesis and aminosugar and nucleotide sugar metabolism.

## Interpreting findings of DE and FA between Transitional and Mature

Protein localization to endoplasmic reticulum is the most significant GO Enrichment term in DE analysis between Transitional and Mature lactation stages (\@ref(tab:TMGOtab)) Scientific research [@HonvoHouto2016] suggest that this is connected with the importance of casein transport, a family of related phosphoproteins that are commonly found in mammalian milk and are synthesized and transported along the secretory pathway and released by exocytosis. Endoplasmic reticulum and the secretory vesicles contribute significantly to the milk fat globule membrane formation and since caseins are secreted by exocytosis there is a need for generating a stable molecular transport system. Second most significant GO term is described as endoplasmic reticulum organization which additionally confirms the importance of this organelle between these two lactation stages. 

KEGG Enrichment between Transitional and Mature lactation stages (\@ref(tab:TMKEGGtab)) didn’t show much difference in comparison to the previous group. It is evident that biosynthesis of glycosphinoglipids and unsaturated fatty acid is still very important with the addition of steroid biosynthesis. A study from 2018 [@Hamdan2018] determined the contents of sterols in the pools of colostrum, transitional and up to 6-month old human milk. Their results showed that the sterol content varies depending on the lactation stage with the highest bio-accessibility in the first lactation stage. 

## Interpreting findings of DE and FA between Colostrum and Mature

GO Enrichment in this groups (Table \@ref(tab:CMGOtab)) showed regulation of autophagy of mitochondrion as a number one GO term. It is known that extreme starvation as a consequence induces the intracellular recycling of nutrients which is needed to sustain energy resources and life. If the starvation is prolonged it becomes destructive, but the induction of autophagy is required for recycling of intracellular contents to stabilize energetic homeostasis under restricted nutrient conditions [@Webster2014]. Intracellular recycling for energetic homeostasis is a requirement that is evident in neonates during the postpartum period where there is not enough fuel in maternal production of milk to fulfil the infant’s energy needs, as stated in [@Kuma2004]. This would suggest that maternal genes could provide products that are regulators of autophagy and are therefore needed in order to ensure the neonates homeostasis once the autophagy begins.

## Cross-referencing with the given article

In comparison with the article [@lemay13] we find similar results. 

### Comparing Colostral with the two other

We also find an DE genes of (iso)ferritins (FTL, FTH1) with enrichment in the GO term "iron ion homeostasis". Furthermore, glycosylation and other sugar modifications are also present (KEGG terms: Glycosphingolipid biosynthesis - globo and isoglobo series, Glycosaminoglycan degradation, Glycosylphosphatidylinositol (GPI)-anchor biosynthesis, Glycan degradation). This processes are probably related to immune functions - antimicrobial role [@Christensen2010]. 

Moreover, several catabolic processes are functionally enriched, suggesting natural pre-digestion of milk proteins [@Dallas2013]. (Tables \@ref(tab:CTGOtab), \@ref(tab:CTKEGGtab), \@ref(tab:CMGOtab), \@ref(tab:CMGKEGGtab))

### Comparing Transitional with the two other

In this stage, we also find that there is an increase in the translational activity and overall ribosomal proteins (KEGG terms: Ribosome, Protein export; GO term: translational initiation). (Tables \@ref(tab:CTGOtab), \@ref(tab:CTKEGGtab), \@ref(tab:TMGOtab), \@ref(tab:TMKEGGtab))

### Comparing Mature with the two other

Finally, in this stage, we also notice the DE of CSN2, a gene encoding milk casein (b-casein) with the GO term "lactation" (Table \@ref(tab:CMGOtab)).

# Conclusions

Not all lactating mothers report that they have achieved their desired amount of breastfeeding for the newborn – they usually include supplements and synthetic milk in the baby’s diet. Breastfeeding provides many benefits, however the physiology of this process remains poorly understood, because of ethical concerns about experimental procedures on breastfeeding mothers. To work around this issue, this paper takes a similar approach to a previous study [@Maningat2009] using micro-array technology, where they isolate RNA from the milk. Moreover, this study is important because it establishes protocols for extracting RNA from milk and processing it for RNA-sequencing technology.

Furthermore, the authors show that, contrary to previous medical belief, the lactational stages do not cluster according to days postpartum, but according to one important biochemical parameter – the ratio of Na:K content. The authors suggest three stages of milk secretion: Colostral (Na:K >= 2.0;), Transitional (Na:K < 2.0) and Mature (Na:K < 0.6). 

When analyzing the MA-plots (Figure \@ref(fig:maPlots)) we found that some samples displayed expression-level dependent bias. This goes hand-in-hand with the fact that:

* because of the sample extraction, it is unknown if the milk fat layer yields RNA purely of mammary epithelial cell origin, or if RNA from non-mammary epithelial sources is also present in the milk fat layer obtained from colostrum, transitional, or mature human milk, and

* because of the body heat and the environment of the milk, RNA might end up being degraded (low RIN values).

Moreover, when we looked at the experimental design and batch identification we could only find one potential batch factor – the protocol used for RNA extraction. However, using hierarchical clustering (Figures \@ref(fig:sampleClusteringTechnique), \@ref(fig:sampleClusteringLacStage)) and a multidimensional scaling plot (Figure \@ref(fig:mdsPlot)) we saw that samples did not group by this technical factor, but rather they grouped by the stage of lactation.

From the obtained results, it is evident that there is a high need for establishing a good immune defense system during the colostral stage of lactation and the production of lipids should be also finished during the mature stage of lactation. During the transitional stage, the up-regulation of massive milk protein synthesis and organization seems to be the most important task. Such results indicate that it is possible to define a lactation stage based on biochemical parameters from RNA expression profiles and these findings are in line with the reported results from the given article [@lemay13] .
