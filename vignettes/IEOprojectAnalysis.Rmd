...
When we analyzed the MA-plots (Figure \@ref(fig:maPlots)) we found that some samples displayed expression-level dependent bias. This goes hand-in-hand with the fact that:
...
Moreover, when we looked at the experimental design and batch identification we could only find one potential batch factor – the protocol used for RNA extraction. However, using hierarchical clustering (Figures \@ref(fig:sampleClusteringTechnique), \@ref(fig:sampleClusteringLacStage)) and a multidimensional scaling plot (Figure \@ref(fig:mdsPlot)) we saw that the samples did not group by this technical factor, rather they grouped by the stage of lactation.
