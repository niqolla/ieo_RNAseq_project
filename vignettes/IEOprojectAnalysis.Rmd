...
Lactation is highly recommended for infants for at least their first year of life, however the breastfeeding success of the mothers is not always achieved. Since the study of this fact is hard to be carried out because of ethical reasons about extracting mammary cells from women in the lactation period, the authors propose the RNA-seq analysis of the surplus feeding milk. They expect to get a deeper understanding of the underlying biological causes of the high variation in mother’s breastfeeding success, specifically they decide to evaluate whether genes related with insulin signaling are linked to breastfeeding ability. 
...
To search for the possible factors, we now explore the column values:
...
-   patients that are repeated because their milk was sampled in 2 or 3
    of the 3 stages of lactation reported in the paper (id numbers: `r `179,
    183, 187).
...
which a gene should be expressed. We are left with `r nrow(se_sample_filtered)` genes.
<!---
We are left with `r nrow(se_sample_filtered)` genes.
...
examining the distribution of samples across the combinations of lactation stage and protocol.

We can see that not all combinations have
...
these two factors. We will have to make comparisons within each lactation stage, between those protocols that have been sequenced.
...
We can see that the samples do not seem to group by protocol. Now we will test if they group by lactation stage in Figure \@ref(fig:sampleClustering_1).


```{r sampleClustering_1, fig.height=5, fig.width=8, dpi=100, echo=FALSE, fig.cap="Figure S6: Hierarchical clustering of the samples. Labels correspond to treatment and sample identifer, while colors indicate sample group."}
par(mar=c(8, 5, 1, 1))
logCPM <- cpm(dge.filt, log=TRUE, prior.count=3)
d <- as.dist(1-cor(logCPM, method="spearman"))
sampleClustering <- hclust(d)
...
plot(sampleDendrogram, main="Hierarchical clustering of samples",
     cex=1.8, cex.axis=1.9, cex.main=1.9)
...
In the progression from colostral to
transitional stages, 8015 genes

7173

and in the
progression from transitional to mature stages, 1948 

1463

Comparing colostral directly to mature, there are 8817 
...
R/Bioconductor package [sva](http://bioconductor.org/packages/sva). We will
compare the expressiona among the lactation stages. We
...
examine the amount of differential expression between the 3 lactation stages.
...
```{r pdistCOV2NHBE, echo=FALSE, out.width="600px", fig.cap="Distribution of raw p-values for an F-test on every gene in the dataset between the 3 lactation stages."}
hist(pv, main="", las=1)
```

The amount of differentially expressed genes is very high, in fact most of the studied genes are. This result might be due to the nature of the analyzed sample, since lactation milk can evolve during the lactation period in order to adapt to the baby's needs. Considering all this the result can be coherent.
