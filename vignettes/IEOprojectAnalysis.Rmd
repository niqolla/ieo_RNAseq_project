...
In the volcano plot below (Figure \@ref(fig:TMvolc)) we analyse the DE genes between Colostrum and Mature

```{r TMvolc, fig.cap="P-value diagnostic plot for differental expression between Colostrum and Mature."}
sv <- sva(assays(se.filt.all)$logCPM, mod=mod, mod0=mod0)
mod <- cbind(mod, sv$sv)
colnames(mod) <- c(colnames(mod)[1:2], paste0("SV", 1:sv$n))
fit3 <- lmFit(assays(se.filt.all)$logCPM, mod)
